...
################################################### Non-linear #########################################
mod2a <- gam(SC3_SC4_logRS ~ s(SC3_SC4_ReproductiveFemales, k=4) + s(Ovig_female_CO,k=4),
             data = dat)
#Model summaries
summary(mod2a)
summary(mod2a$gam)
...
################################################### Non-linear ############################################
mod3a <- gam(SC3_SC4_logRS ~ s(SC3_SC4_ReproductiveFemales, k=4) + s(Pcod_lag1,k=4),
             data = dat)
#Model summaries
summary(mod3a)
summary(mod3a$gam)
...
################################### non-linear ##############################################
mod4a <- gamm(SC3_SC4_logRS ~ s(SC3_SC4_ReproductiveFemales, k=4) + s(PCod_RA2,k=4),
             data = dat, correlation=corAR1())
#Model summaries
summary(mod4a)
summary(mod4a$gam)
...
################################ non-linear ######################################################
mod29a <- gamm(SC3_SC4_logRS ~ s(SC3_SC4_ReproductiveFemales, k=4) + s(FHS_lag2, k=4) + s(PDO_RA2,k=4),
              data = dat, correlation=corAR1())
...
###################################################### nonlinear #####################################################
mod34a <- gam(SC3_SC4_logRS ~ s(SC3_SC4_ReproductiveFemales, k=4) + s(FHS_lag2, k=4) + s(PDO_RA2, k = 4) + s(AO_RA2,k=4),
              data = dat)
...
write.csv(AICc_linear, "output/Mixed_GAM_linear_model_AICc_values_SC3_SC4.csv")
