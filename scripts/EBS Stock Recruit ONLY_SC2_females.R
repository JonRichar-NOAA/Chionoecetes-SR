...
names(Spawners_SC2)
Spawners_SC2
W166.fem<-Spawners_SC2$W166_SC2
EBS.fem<-Spawners_SC2$EBS_SC2
E166.fem<-Spawners_SC2$E166_SC2
plot(EBS.fem)
W166.fem
...
title(main=" SC2 female abundance by year")



#########################################################################################################################
# These are primiparous crab--will need to add one year to lag to account for fact that contribution to output will not occur until next year

#########################################################################################################################
########################################### Lag=3 #######################################################################
par(mfrow=c(2,2))

plot(rec_30to50$EBS_Abun_30to50[4:45]~rec_30to50$Year[4:45], ylab="Estimated juvenile abundance", xlab="Year", pch=16)
plot(Spawners_SC2$EBS_SC2~Spawners_SC2$Year, ylab="Estimated reproductive female crab abundance", xlab="Year",pch=16)

par(mfrow=c(2,2))
R<-rec_30to50$EBS_Abun_30to50[4:45]
S<-Spawners_SC2$EBS_SC2[4:45]
log.R<-log(R)
log.S<-log(S)


xi<-S
xyear<-Spawners_SC2$Year[4:45]
yi<-R
...
plot(mod0$gam, resid=T, pch=19, rug=F, se=F, pages=1)
plot(mod0$lme, resid=T, pch=19, rug=F, se=F, pages=1)
MuMIn::AICc(mod0)
...
summary(Fit3)
MuMIn::AICc(Fit3)
plot(log(R/S)~S, xlab="Reproductive female abundance",ylab="lag 3 log(Recruits/Repfem)",main="EBS group lag 3 Ricker model-SC2 females only",pch=16)
abline(Fit3)
