...
rec_30to50
rec_30to50$Year[4:48]
par(mfrow=c(2,2),cex.lab=1.25,cex.axis=1.25,cex=1.25) #configure axis labels)

R<-rec_30to50$EBS_Abun_30to50[4:48]
S<-Spawners_SC3$EBS_SC3
...
axis(1, at=1:48,lab=Spawners_SC3$Year,tck=0.03)
...
EBS.Recruits<-read.csv("./data/cb_ebs_pop_juvenile.csv")
W166.Recruits<-read.csv("./data/cb_w166_pop_juvenile.csv")
E166.Recruits<-read.csv("./data/cb_e166_pop_juvenile.csv")

names(EBS.Recruits)
names(W166.Recruits)
names(E166.Recruits)

W166.juv<-W166.Recruits$Juv_abun
E166.juv<-E166.Recruits$Juv_abun
EBS.juv<-EBS.Recruits$Juv_abun
...
axis(1, at=1:48,lab=Recruits$Year,tck=0.02)
...
plot(R2~S2,xlab= "Reproductive female abundance", ylab="Juvenile abundance",main= "EBS juveniles vs NS females",pch=16)
Fit3<-gls(log(R2/S2)~S2,correlation=corAR1(), na.action=na.omit)
summary(Fit3)
plot(log(R2/S2)~S2, xlab="Reproductive female abundance",ylab="LN(Recruits/Repfem)",main="EBS log-survival vs. abundance of SC2 females",pch=16)
abline(Fit3)
#plot(Fit3)
...
plot(r.fit3,pch=16,xlab="Hatch year", ylab=" S/R residuals", main=" EBS lag 2 S/R residuals vs. hatch year")
...
Fit3<-gls(log(R/S)~S,correlation=corAR1(), na.action=na.omit)
summary(Fit3)
plot(log(R/S)~S, xlab="Reproductive female abundance",ylab="lag 3 log(Recruits/Repfem)",main="EBS group lag 3 Ricker model-SC3 females only",pch=16)
abline(Fit3)
#plot(Fit3)
...
Fit3<-gls(log(R2/S2)~S2,correlation=corAR1(), na.action=na.omit)
summary(Fit3)
plot(log(R2/S2)~S2, xlab="Reproductive female abundance",ylab="LN(Recruits/Repfem)",main="EBS log-survival vs. abundance of reproductive females-SC3 only",pch=16)
abline(Fit3)
#plot(Fit3)
