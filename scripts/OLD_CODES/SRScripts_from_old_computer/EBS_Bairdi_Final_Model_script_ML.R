...
#write.csv(dat,"data/EBS_Crab_and_envar_data_for_analysis.csv")
####################################################################################################################################################################
############################################## FIT MODELS AND COMPARE ##############################################################################################

############################## set up a reduced data frame with data ##############################################
dat <- data.frame(releaseyear = c(1983:2016),
                  recr = R1 ,
                  spawners = S1,
                  logRS = log(R1/S1),
...
fit1<-gls(logRS ~ log(spawners) + FHS, correlation = corAR1(), method="ML", data = dat)
summary(fit1)
# phi = 0.48
