...
year.fhs2<-All_Dat$releaseyear[5:length(All_Dat$releaseyear)]
year
year.o
year.fhs
...
r.fit42<-loess(r42~year.o,span=0.4)
r.fit43<-loess(r43~year,span=0.4)
r.fit44<-loess(r44~year.fhs,span=0.4)
...
fit22<-gls(r ~ Fem_CO, correlation = corAR1(), method="ML",na.action = na.omit)
fit23<-gls(r ~ cod, correlation = corAR1(), method="ML")
fit24<-gls(r ~ fhs, correlation = corAR1(), method="ML",na.action = na.omit)
...
fit32<-gls(r ~ s+ Fem_CO, correlation = corAR1(), method="ML",na.action=na.omit)
fit33<-gls(r ~ s+ cod, correlation = corAR1(), method="ML")
fit34<-gls(r ~ s+ fhs, correlation = corAR1(), method="ML",na.action=na.omit)
...
year_7<-year[8:length(r)]

mod_7 <- gam(logR7 ~ s(S7, k = 4)+s(Rminus7, k = 4),
             data = dat, method = "REML")
summary(mod_7)
